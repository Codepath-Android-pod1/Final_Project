...
class EventAdapter(private val context: Context, private val events: MutableList<Event>) :
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(event: Event) {
            val parsedDate = LocalDateTime.parse(event.dates.start.dateTime)
            val venue = event._embedded.venues[0]
            val location = "${venue.name} - ${venue.city}, ${venue.state}"
            tvTitle.text = event.name
            tvDate.text = parsedDate.format(DateTimeFormatter.ofPattern("M d E h:mma"))
